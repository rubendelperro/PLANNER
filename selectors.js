import { aggregateNutritionalVectors, PRECISION_FACTOR, SLOTS, getNutrientVectorForPlannedItem as utilGetNutrientVectorForPlannedItem, getWeekDates } from './utils.js';
...
export function calculateRecipeTotals(recipe, itemsById) {
    ...
    const vectors = recipe.ingredients.map(ri => {
    ...
    const totalGrams = recipe.ingredients.reduce((sum, ing) => sum + ing.grams, 0);
...
export function calculateBaseTargets(profile) {
    ...
    if (!profile || profile.isDefault || !profile.age || !profile.weight || !profile.height || !profile.gender || !profile.activityLevel || !profile.goal) {
        return {};
    }
    const { age, weight, height, gender, activityLevel, goal } = profile;
    const activityMultipliers = { sedentary: 1.2, light: 1.375, moderate: 1.55, high: 1.725, very_high: 1.9 };
    ...
    if (referenceGuide && referenceGuide.nutrients) {
        for (const nutrientId in referenceGuide.nutrients) {
            scaledTargets[nutrientId] = referenceGuide.nutrients[nutrientId] * scalingFactor;
        }
    }
    ...
    if (goal === 'lose') {
        ...
        carbGrams = Math.round((targetCalories * 0.30) / 4);
    } else if (goal === 'gain') {
...
export function getActiveNutritionalTargets() {
    ...
    const referenceGuide = state.referenceGuides.byId[activeProfile.referenceSourceId];
    ...
    for (const nutrientId of trackedNutrients) {
        ...
        const personalValue = activeProfile.personalGoals ? activeProfile.personalGoals[nutrientId] : undefined;
...
export function getRecipeWithDetails(itemId) {
    ...
    if (!item || item.itemType !== 'receta' || !item.computed) { return null; }
...
function getNutrientVectorForPlannedItem(plannedItem, state) {
    return utilGetNutrientVectorForPlannedItem(plannedItem, state, getNutritionalVectorForItem);
}
...
export function getDayAnalysis(date) {
    ...
    for (const nutrientId of trackedNutrients) {
        ...
        const percentage = targetValue != null && targetValue > 0 ? Math.min(100, Math.round((value / targetValue) * 100)) : 0;
        ...
        analysis.push({
            nutrientId, name: definition.name, unit: definition.unit,
...
export function getWeekAnalysis() {
    ...
    const signature = `${ui.activePlannerDay}-${ui.activeProfileId}-` +
                    `${items.allIds.length}-` +
                    JSON.stringify(weekPlannerData);

    if (_memoizationCache.weekAnalysis && _memoizationCache.weekAnalysis.signature === signature) {
...
export function getShoppingList() {
                ...
                for (const plannedItem of planner[date][slot]) {
                    ...
                    if (itemDetails.itemType === 'receta') {
                        ...
                        for (const recipeIngredient of itemDetails.ingredients) {
                            ...
                            const currentAmount = requiredIngredients.get(recipeIngredient.ingredientId) || 0;
                            requiredIngredients.set(recipeIngredient.ingredientId, currentAmount + requiredAmount);
                        }
                    } else if (itemDetails.itemType === 'ingrediente') {
                        const currentAmount = requiredIngredients.get(plannedItem.id) || 0;
                        requiredIngredients.set(plannedItem.id, currentAmount + plannedItem.grams);
                    }
                }
            }
        }
    ...
    for (const [ingredientId, totalGrams] of requiredIngredients.entries()) {
        ...
        if (ingredientDetails) {
            const itemForList = { ...ingredientDetails, totalGrams: parseFloat(totalGrams.toFixed(2)) };
...
export function getAggregatedAnalysis() {
    ...
    for (const nutrientId of trackedNutrients) {
        ...
        const adjustedTarget = dailyTargetValue != null ? dailyTargetValue * targetMultiplier : null;

        const percentage = adjustedTarget != null && adjustedTarget > 0 ? Math.min(100, Math.round((value / adjustedTarget) * 100)) : 0;
        ...
        finalAnalysis.push({
            nutrientId, name: definition.name, unit: definition.unit,
            value: parseFloat(value.toFixed(2)),
            target: adjustedTarget != null ? parseFloat(adjustedTarget.toFixed(2)) : null,
            percentage, status,
        });
    }
...
export function getInferredItemMetrics(itemId) {
    const state = getState();
    const item = state.items.byId[itemId];
    if (!item || !item.logistics || !item.logistics.purchaseInfo || !item.logistics.price || !item.logistics.purchaseInfo.packageValue > 0) {
        return null;
    }
...
export function getNutritionalVectorForItem(item, amount, unit = 'gramos') {
    ...
    } else if (item.itemType === 'receta' && item.computed && item.computed.totalGrams > 0) {
        scalingFactor = totalGrams / item.computed.totalGrams;
    }

    const scaledNutrients = {};
    const sourceNutrients = item.itemType === 'receta' ? item.computed.totals : item.nutrients;
...
export function findDependentRecipes(itemId) {
    ...
    return state.items.allIds
        ...
        .filter(item => item.itemType === 'receta' && item.ingredients?.some(ing => ing.ingredientId === itemId))
