...
// Control center moved to components/ControlCenter.js

// Nutrient manager renderer extracted to components/NutrientManager.js
  ...
  renderSettingsView(state) {
    ...
    return `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="col-span-1">${renderControlCenter(state)}</div>
                  ...
                  .map((storeId, index) => {
                    ...
                    <label class="flex items-center ${isPrimary ? 'bg-blue-50 p-2 rounded' : ''}">
                        ...
                        <span class="ml-2 text-sm text-gray-600">
                            ${store.name}
                            ${isPrimary ? '<span class="text-xs text-blue-600 font-semibold"> (Principal)</span>' : ''}
                        </span>
                    </label>
                    `;
                  })
  ...
  return `
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            ...
            <form id="add-store-form" class="flex gap-4 mb-4">
                <input type="text" name="storeName" placeholder="Nombre del supermercado" required class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 text-sm">Añadir</button>
            </form>
                  ...
                  .map((storeId) => {
                    ...
                    return `
                        <div class="flex justify-between items-center p-2 bg-gray-50 rounded-md">
                            <div class="text-sm">${store?.name || 'Supermercado desconocido'}</div>
                            <div>
  ...
  return `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-1 space-y-8">
                <form id="profile-form" class="space-y-3 border-b pb-4 mb-8">
