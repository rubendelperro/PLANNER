  ...
  return `
        <div class="bg-white p-6 rounded-lg shadow-md">
          ...
          <div class="flex mb-4 bg-gray-200 rounded-lg p-1">
            <button id="per-package-btn" class="flex-1 py-2 px-4 text-sm font-medium rounded-md text-gray-600 hover:text-gray-900">Todo el paquete</button>
            <button id="per-serving-btn" class="flex-1 py-2 px-4 text-sm font-medium rounded-md text-gray-600 hover:text-gray-900">1 ración</button>
            <button id="per-unit-btn" class="flex-1 py-2 px-4 text-sm font-medium rounded-md text-gray-600 hover:text-gray-900">1 unidad</button>
            <button id="per-100g-btn" class="flex-1 py-2 px-4 text-sm font-medium rounded-md bg-white text-gray-900 shadow">100g contienen</button>
          </div>
                      ...
                      trackedNutrients.length > 0
                        ? trackedNutrients
                            .map((nutrientId) => {
                              ...
                              // Calcular valores por paquete/unidad si hay logística
                              const packageGrams =
                                nutrient.logistics?.packageGrams ??
                                nutrient.logistics?.purchaseInfo?.packageValue;
                              const units =
                                nutrient.logistics?.unitsPerPackage ??
                                nutrient.logistics?.purchaseInfo?.servingCount;
                              // Note: recipe-level logistics may exist on `recipe.logistics`
                              const recipePackageGrams =
                                recipe.logistics?.packageGrams ??
                                recipe.logistics?.purchaseInfo?.packageValue;
                              const recipeUnits =
                                recipe.logistics?.unitsPerPackage ??
                                recipe.logistics?.purchaseInfo?.servingCount;
                              let perPackageDisplay = '';
                              let perUnitDisplay = '';
                              if (recipePackageGrams && totalGrams > 0) {
                                perPackageDisplay = parseFloat(
                                  (
                                    per100gValue *
                                    (recipePackageGrams / 100)
                                  ).toFixed(2)
                                );
                              }
                              if (recipeUnits && recipePackageGrams) {
                                ...
                                perUnitDisplay = parseFloat(
                                  (per100gValue * (gramsPerUnit / 100)).toFixed(
                                    2
                                  )
                                );
                              }
                                ...
                                <div class="space-y-2">
                                    <div class="flex justify-between font-medium">
                                        ...
                                        <span class="nutrient-value"
                                              ...
                                              data-perpackage="${perPackageDisplay || ''}"
                                              data-perunit="${perUnitDisplay || ''}"
  ...
  return `
        ...
        <span class="font-bold text-gray-900 nutrient-value"
          ...
          data-unit="${definition.unit || ''}"
                ...
                <!-- Mostrar TODOS los nutrientes (definiciones) para permitir configuración posterior -->
                <div class="mt-6 bg-white p-6 rounded-lg border">
                    <h4 class="font-semibold text-gray-700 mb-3">100g contienen</h4>
                    <div class="space-y-2 text-sm">
                        ${
                          // Iterate all nutrient definition items (itemType === 'definicion')
                          items.allIds
                            .filter(
                            ...
                            .map((nutrientId) => {
                              ...
                              if (
                                ...
                                if (
                                  computedTotals &&
                                  computedTotals[nutrientId] != null &&
                                  computedGrams > 0
                                ) {
                                  const scalingFactor = 100 / computedGrams;
                                  value100g =
                                    computedTotals[nutrientId] * scalingFactor;
                                }
                              }
                              ...
                              if (packageGrams && packageGrams > 0) {
                                perPackage = parseFloat(
                                  (displayValue * (packageGrams / 100)).toFixed(
                                    2
                                  )
                                );
                              }
                              if (units && packageGrams) {
                                const gramsPerUnit = packageGrams / units;
                                perUnit = parseFloat(
                                  (displayValue * (gramsPerUnit / 100)).toFixed(
                                    2
                                  )
                                );
                              }
                              ...
                              return `
                                <div class="space-y-2">
                                    ...
                                    `
                                            : `
                                        <span class="font-bold text-gray-900 nutrient-value" data-per100g="${displayValue}" data-perserving="" data-perpackage="${perPackage || ''}" data-perunit="${perUnit || ''}" data-unit="${def.unit || ''}">${displayValue}${def.unit || ''}</span>
                                    `
                                        }
                                    </div>
                                </div>
