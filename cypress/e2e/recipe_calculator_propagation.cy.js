...
describe('Recipe calculator propagation', () => {
  it('computes recipe totals and propagates them across planner and profiles', () => {
    ...
    cy.window().then(async (win) => {
      ...
      cy.createProfile(userA);
      cy.createProfile(userB);
      ...
      cy.dispatch({ type: 'ADD_RECIPE', payload: recipe });

      cy.getState().then((stateAfterRecipe) => {
        ...
        function getItemTotals(item) {
          ...
          if (item.computed && item.computed.totals)
            return item.computed.totals;
        ...
        cy.getState().then((s) => {
          ...
          // Continue the rest of the test inside this getState.then chain (below)

          // 3) Planner & Propagation Phase (UserA)
          // Set active profile to UserA
          cy.dispatch({ type: 'SET_ACTIVE_PROFILE', payload: userA.id });
          ...
          cy.getState().then((stateAfterPlan) => {
            ...
            Object.keys(plannerExpectedTotals).forEach((k) => {
              ...
              const canonical =
                (expectedRecipeTotals[k] || 0) * scaleForThisRecipe;
              expect(Number.isFinite(exp), `planner expected ${k} is finite`).to
                .be.true;
            ...
            cy.dispatch({ type: 'SET_ACTIVE_PROFILE', payload: userB.id });
            cy.getState().then((stateAfterSwitchB) => {
              const entriesB = findEntriesById(
              ...
              expect(
                hasEntryB,
                'UserB sees the planned recipe (planner is global)'
              ).to.be.true;
              ...
              if (standardId) {
                ...
                cy.getState().then((stateAfterStd) => {
                  expect(stateAfterStd, 'state after switching to default').to
                    .exist;
                });
              }
              ...
              cy.dispatch({ type: 'SET_ACTIVE_PROFILE', payload: userA.id });
              cy.getState().then((stateBackToA) => {
                const entriesA2 = findEntriesById(
                  stateBackToA.planner,
                  recipeId
                );
                const foundAgain =
                ...
                expect(
                  foundAgain,
                  'recipe returns for UserA after switching back'
                ).to.exist;
              });
            });
          });
