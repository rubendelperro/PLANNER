import { Logger, loadState, saveState, debounce, SLOTS, createRecipeEditorState } from './utils.js';
    ...
    switch (action.type) {
        ...
        case 'DELETE_PROFILE': {
            ...
            if (draftState.profiles.byId[profileIdToDelete]?.isDefault) {
                Logger.warn('Attempted to delete the default profile. Operation aborted.');
                break;
            }
            delete draftState.profiles.byId[profileIdToDelete];
            draftState.profiles.allIds = draftState.profiles.allIds.filter(id => id !== profileIdToDelete);
        ...
        case 'UPDATE_PROFILE': {
            ...
            if (dataToUpdate.age !== undefined) dataToUpdate.age = Math.max(0, parseFloat(dataToUpdate.age) || 0);
            if (dataToUpdate.weight !== undefined) dataToUpdate.weight = Math.max(0, parseFloat(dataToUpdate.weight) || 0);
            if (dataToUpdate.height !== undefined) dataToUpdate.height = Math.max(0, parseFloat(dataToUpdate.height) || 0);
        ...
        case 'UPDATE_INGREDIENT_LOGISTICS': {
            ...
            if (!ingredient) break;
            if (stockInGrams !== undefined) ingredient.logistics.stock.value = stockInGrams;
        ...
        case 'DELETE_ITEM': {
            ...
            if (item && item.itemType === 'definicion') {
                ...
                for (const profileId of draftState.profiles.allIds) {
                    ...
                    if (profile.trackedNutrients) {
                        profile.trackedNutrients = profile.trackedNutrients.filter(id => id !== itemIdToDelete);
                    }
                    if (profile.personalGoals && profile.personalGoals[itemIdToDelete] !== undefined) {
            ...
            draftState.items.allIds = draftState.items.allIds.filter(id => id !== itemIdToDelete);
            break;
        }
        case 'DELETE_TAG': {
            const tagIdToDelete = action.payload;
            delete draftState.tags.byId[tagIdToDelete];
            draftState.tags.allIds = draftState.tags.allIds.filter(id => id !== tagIdToDelete);
            break;
        }
        case 'ADD_STORE': {
        ...
        case 'DELETE_STORE': {
            ...
            draftState.stores.allIds = draftState.stores.allIds.filter(id => id !== storeIdToDelete);
            ...
            for (const itemId of draftState.items.allIds) {
                ...
                if (item.logistics && item.logistics.preferredStoreIds) {
                    item.logistics.preferredStoreIds = item.logistics.preferredStoreIds.filter(id => id !== storeIdToDelete);
                }
            }
            break;
        }
        ...
        case 'DELETE_CATEGORY': {
            ...
            draftState.categories.allIds = draftState.categories.allIds.filter(id => id !== categoryIdToDelete);

            // Remover la categoría de todos los ingredientes que la usen
            for (const itemId of draftState.items.allIds) {
                const item = draftState.items.byId[itemId];
                if (item.categoryIds) {
                    item.categoryIds = item.categoryIds.filter(id => id !== categoryIdToDelete);
                }
            }
            break;
        }
        case 'ADD_RECIPE': {
            ...
            const { computedTotals, totalGrams } = calculateRecipeTotals(newRecipeData, draftState.items.byId);
        ...
        case 'ASSIGN_ITEM_TO_SLOT': {
            ...
            const datePlan = draftState.planner[date] || { breakfast: [], lunch: [], dinner: [], snacks: [] };
        ...
        case 'REMOVE_PLANNED_MEAL': {
            ...
            if (draftState.planner[date] && draftState.planner[date][slot] &&
                itemIndex >= 0 && itemIndex < draftState.planner[date][slot].length) {
        ...
        case 'SET_ACTIVE_DAY': {
            ...
            draftState.ui.activePlannerDay = newActiveDayId === draftState.ui.activePlannerDay ? null : newActiveDayId;
            break;
        }
        ...
        case 'UPDATE_RECIPE_SERVINGS': {
            ...
            if (recipeEditor.isEditing && recipeEditor.recipeId === action.payload.recipeId) {
        ...
        case 'REMOVE_INGREDIENT_FROM_EDITOR': {
            draftState.ui.recipeEditor.ingredients = draftState.ui.recipeEditor.ingredients.filter(
        ...
        case 'SAVE_RECIPE_EDITS': {
            ...
            if (recipe) {
                ...
                const { computedTotals, totalGrams } = calculateRecipeTotals(recipe, draftState.items.byId);
                recipe.computed = { totals: computedTotals, totalGrams };
            }
        ...
        case 'COMPLEX_TOGGLE': {
            ...
            const currentMonth = isMonthView ? new Date(draftState.ui.activePlannerDay).getMonth() : null;
            ...
            if (type === 'cell') {
                ...
                const dateStr = value;
                if (!isMonthView || new Date(dateStr + "T12:00:00").getMonth() === currentMonth) {
                    SLOTS.forEach(slot => {
            ...
            const allIdsAreSelected = idsToToggle.size > 0 && Array.from(idsToToggle).every(id => draftState.ui.selectedCells.has(id));
        ...
        case 'CHANGE_WEEK': {
            ...
            const currentDate = new Date(draftState.ui.activePlannerDay || new Date());
        ...
        case 'REMOVE_NOTIFICATION': {
            draftState.ui.notifications = draftState.ui.notifications.filter(n => n.id !== action.payload);
            break;
        }
        ...
        case 'OPEN_DELETE_ITEM_MODAL': {
            ...
            draftState.ui.deleteItemModal = { isOpen: true, itemId, dependentRecipes };
            break;
        }
        case 'CLOSE_DELETE_ITEM_MODAL':
            draftState.ui.deleteItemModal = { isOpen: false, itemId: null, dependentRecipes: [] };
            break;
        case 'DELETE_INGREDIENT': {
            const itemId = action.payload;
            // Eliminar de items
            delete draftState.items.byId[itemId];
            draftState.items.allIds = draftState.items.allIds.filter(id => id !== itemId);
            ...
            for (const recipeId of draftState.items.allIds) {
                ...
                if (recipe.itemType === 'receta' && recipe.ingredients) {
                    ...
                    recipe.ingredients = recipe.ingredients.filter(ing => ing.ingredientId !== itemId);
            ...
            for (const dateKey in draftState.planner) {
                for (const slot in draftState.planner[dateKey]) {
                    draftState.planner[dateKey][slot] = draftState.planner[dateKey][slot].filter(item => item.id !== itemId);
                }
            }
...
function migrateStateTo_20_0_0(legacyState) {
    try {
        ...
        const newState = {
            referenceGuides: {
                byId: {
                    'EFSA-2017': { id: 'EFSA-2017', name: 'Guía de Referencia EFSA 2017', nutrients: { 'calories': 2000, 'proteins': 50, 'fats': 70, 'carbs': 260 } },
                    'FDA-2020': { id: 'FDA-2020', name: 'Guía de Referencia FDA 2020', nutrients: { 'calories': 2000, 'proteins': 50, 'fats': 78, 'carbs': 275 } }
                },
        ...
        if (stateCopy.ingredients) {
            for (const id of stateCopy.ingredients.allIds) {
                ...
                newState.items.byId[id] = {
                    ...
                    tags: [],
                    logistics: {
                        stock: { value: ingredient.logistics?.stockInGrams || 0, unit: 'gramos' },
                        price: ingredient.logistics?.prices?.length > 0 ? ingredient.logistics.prices[ingredient.logistics.prices.length - 1] : {}
                    }
                };
                newState.items.allIds.push(id);
            }
...
export function init() {
    ...
    let initialData = {
        referenceGuides: {
            byId: {
                'EFSA-2017': {
                    ...
                    nutrients: {
                        'calories': 2000, 'proteins': 50, 'fats': 70, 'carbs': 260,
                        'vitamin_c': 80, 'calcium': 800, 'iron': 11, 'potassium': 2000, 'sodium': 2000
                    }
                },
                'FDA-2020': {
                    id: 'FDA-2020',
                    ...
                    nutrients: {
                        'calories': 2000, 'proteins': 50, 'fats': 78, 'carbs': 275,
                        'vitamin_d': 20, 'calcium': 1300, 'iron': 18, 'potassium': 4700, 'sodium': 2300
                    }
                }
            },
            allIds: ['EFSA-2017', 'FDA-2020']
        ...
        stores: {
            byId: {
                ...
                'STORE-SUPERMERCADO': { id: 'STORE-SUPERMERCADO', name: 'Supermercado' },
            },
        ...
        categories: {
            ...
            allIds: ['CAT-CARNES', 'CAT-PESCADOS', 'CAT-FRUTAS', 'CAT-VERDURAS', 'CAT-DULCES', 'CAT-LEGUMBRES', 'CAT-CEREALES', 'CAT-LACTEOS']
        },
        tags: {
            byId: {}, allIds: []
        },
        items: {
            byId: {
                ...
                'calories': { id: 'calories', name: 'Calorías', unit: 'kcal', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'proteins': { id: 'proteins', name: 'Proteínas', unit: 'g', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'carbs': { id: 'carbs', name: 'Carbohidratos', unit: 'g', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'fats': { id: 'fats', name: 'Grasas', unit: 'g', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'vitamin_c': { id: 'vitamin_c', name: 'Vitamina C', unit: 'mg', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'vitamin_d': { id: 'vitamin_d', name: 'Vitamina D', unit: 'mcg', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'calcium': { id: 'calcium', name: 'Calcio', unit: 'mg', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'iron': { id: 'iron', name: 'Hierro', unit: 'mg', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'potassium': { id: 'potassium', name: 'Potasio', unit: 'mg', type: 'nutriente', itemType: 'definicion', isCustom: false },
                'sodium': { id: 'sodium', name: 'Sodio', unit: 'mg', type: 'nutriente', itemType: 'definicion', isCustom: false },

                // INGREDIENTS
                'CHICKEN-BREAST': {
                    id: 'CHICKEN-BREAST', name: 'Pechuga de Pollo', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 165, 'proteins': 31, 'fats': 3.6, 'carbs': 0, 'vitamin_c': 0, 'vitamin_d': 0, 'calcium': 11, 'iron': 1, 'potassium': 256, 'sodium': 74, 'servingSizeGrams': 100 },
                    tags: [], categoryIds: ['CAT-CARNES'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { }, preferredStoreIds: [] }
                },
                'MANZANA-FUJI': {
                    id: 'MANZANA-FUJI', name: 'Manzana Fuji', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 52, 'proteins': 0.3, 'fats': 0.2, 'carbs': 14, 'vitamin_c': 4.6, 'vitamin_d': 0, 'calcium': 6, 'iron': 0.1, 'potassium': 107, 'sodium': 1, 'servingSizeGrams': 180 },
                    tags: [], categoryIds: ['CAT-FRUTAS'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { }, preferredStoreIds: [] }
                },
                'ESPINACAS-FRESCAS': {
                    id: 'ESPINACAS-FRESCAS', name: 'Espinacas Frescas', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 23, 'proteins': 2.9, 'fats': 0.4, 'carbs': 3.6, 'vitamin_c': 28, 'vitamin_d': 0, 'calcium': 99, 'iron': 2.7, 'potassium': 558, 'sodium': 79, 'servingSizeGrams': 85 },
                    tags: [], categoryIds: ['CAT-VERDURAS'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 1.50 }, preferredStoreIds: ['STORE-MERCADONA'], purchaseInfo: { packageValue: 250, unit: 'gramos', servingCount: 2.94 } }
                },
                'SALMON-FILLET': {
                    id: 'SALMON-FILLET', name: 'Filete de Salmón', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 208, 'proteins': 20, 'fats': 13, 'carbs': 0, 'vitamin_c': 0, 'vitamin_d': 13.6, 'calcium': 9, 'iron': 0.3, 'potassium': 363, 'sodium': 59, 'servingSizeGrams': 140 },
                    tags: [], categoryIds: ['CAT-PESCADOS'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 8.99 }, preferredStoreIds: ['STORE-MERCADONA'], purchaseInfo: { packageValue: 280, unit: 'gramos', servingCount: 2 } }
                },
                'PATATA-GRANDE': {
                    id: 'PATATA-GRANDE', name: 'Patata Grande', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 77, 'proteins': 2, 'fats': 0.1, 'carbs': 17, 'vitamin_c': 19.7, 'vitamin_d': 0, 'calcium': 12, 'iron': 0.8, 'potassium': 429, 'sodium': 6, 'servingSizeGrams': 200 },
                    tags: [], categoryIds: ['CAT-VERDURAS'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { }, preferredStoreIds: [] }
                },
                'GALLETAS-CHOCOLATE': {
                    id: 'GALLETAS-CHOCOLATE', name: 'Galletas con Chocolate', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 502, 'proteins': 5.3, 'fats': 24, 'carbs': 68, 'vitamin_c': 0, 'vitamin_d': 0, 'calcium': 20, 'iron': 2, 'potassium': 150, 'sodium': 170, 'servingSizeGrams': 20 },
                    tags: [], categoryIds: ['CAT-DULCES'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 2.10 }, preferredStoreIds: ['STORE-SUPERMERCADO'], purchaseInfo: { packageValue: 200, unit: 'gramos', servingCount: 10 } }
                },
                'DORITOS-TEX-MEX': {
                    id: 'DORITOS-TEX-MEX', name: 'Doritos Tex-Mex', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 498, 'proteins': 6.5, 'fats': 26, 'carbs': 57, 'vitamin_c': 0, 'vitamin_d': 0, 'calcium': 0, 'iron': 0, 'potassium': 0, 'sodium': 810, 'servingSizeGrams': 30 },
                    tags: [], categoryIds: ['CAT-DULCES'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 1.85 }, preferredStoreIds: ['STORE-SUPERMERCADO'], purchaseInfo: { packageValue: 170, unit: 'gramos', servingCount: 5.66 } }
                },
                'ARROZ-BLANCO': {
                    id: 'ARROZ-BLANCO', name: 'Arroz Blanco (crudo)', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 365, 'proteins': 7.1, 'fats': 0.7, 'carbs': 80, 'vitamin_c': 0, 'vitamin_d': 0, 'calcium': 28, 'iron': 0.8, 'potassium': 115, 'sodium': 5, 'servingSizeGrams': 45 },
                    tags: [], categoryIds: ['CAT-CEREALES'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 1.20 }, preferredStoreIds: ['STORE-SUPERMERCADO'], purchaseInfo: { packageValue: 1000, unit: 'gramos', servingCount: 22.22 } }
                },
                'ATUN-LATA-ACEITE': {
                    id: 'ATUN-LATA-ACEITE', name: 'Atún en Lata (aceite, escurrido)', type: 'alimento', itemType: 'ingrediente',
                    nutrients: { 'calories': 184, 'proteins': 29, 'fats': 7, 'carbs': 0, 'vitamin_c': 0, 'vitamin_d': 2, 'calcium': 10, 'iron': 1.5, 'potassium': 257, 'sodium': 392, 'servingSizeGrams': 52 },
                    tags: [], categoryIds: ['CAT-PESCADOS'], logistics: { stock: { value: 0, unit: 'gramos' }, price: { value: 2.50 }, preferredStoreIds: ['STORE-SUPERMERCADO'], purchaseInfo: { packageValue: 156, unit: 'gramos', servingCount: 3 } }
                },

                // RECIPES
                'REC-ENSALADA-ESPINACAS': {
                    id: 'REC-ENSALADA-ESPINACAS', name: 'Ensalada de Espinacas y Manzana', type: 'alimento', itemType: 'receta',
                    ingredients: [ { ingredientId: 'ESPINACAS-FRESCAS', grams: 120 }, { ingredientId: 'MANZANA-FUJI', grams: 80 } ], // Total: 200g
                    tags: [], computed: {}
                },
                'REC-SALMON-PATATAS': {
                    id: 'REC-SALMON-PATATAS', name: 'Salmón con Patatas', type: 'alimento', itemType: 'receta',
                    ingredients: [ { ingredientId: 'SALMON-FILLET', grams: 150 }, { ingredientId: 'PATATA-GRANDE', grams: 200 } ], // Total: 350g
                    tags: [], computed: {}
                },
                'REC-POLLO-ARROZ': {
                    id: 'REC-POLLO-ARROZ', name: 'Pollo con Arroz', type: 'alimento', itemType: 'receta',
                    ingredients: [ { ingredientId: 'CHICKEN-BREAST', grams: 120 }, { ingredientId: 'ARROZ-BLANCO', grams: 80 } ], // Total: 200g
                    tags: [], computed: {}
                },
                'REC-ENSALADA-ATUN': {
                    id: 'REC-ENSALADA-ATUN', name: 'Ensalada de Atún', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'ATUN-LATA-ACEITE', grams: 100 }, { ingredientId: 'ESPINACAS-FRESCAS', grams: 100 } ], // Total: 200g
                    tags: [], computed: {}
                },
                'REC-SALMON-ESPINACAS': {
                    id: 'REC-SALMON-ESPINACAS', name: 'Salmón con Espinacas', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'SALMON-FILLET', grams: 140 }, { ingredientId: 'ESPINACAS-FRESCAS', grams: 110 } ], // Total: 250g
                    tags: [], computed: {}
                },
                'REC-POLLO-PATATAS': {
                    id: 'REC-POLLO-PATATAS', name: 'Pollo con Patatas Asadas', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'CHICKEN-BREAST', grams: 130 }, { ingredientId: 'PATATA-GRANDE', grams: 170 } ], // Total: 300g
                    tags: [], computed: {}
                },
                'REC-ARROZ-ATUN': {
                    id: 'REC-ARROZ-ATUN', name: 'Arroz con Atún', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'ARROZ-BLANCO', grams: 70 }, { ingredientId: 'ATUN-LATA-ACEITE', grams: 80 } ], // Total: 150g
                    tags: [], computed: {}
                },
                'REC-ENSALADA-POLLO': {
                    id: 'REC-ENSALADA-POLLO', name: 'Ensalada de Pollo y Manzana', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [
                ...
                'REC-PATATA-ATUN': {
                    id: 'REC-PATATA-ATUN', name: 'Patatas con Atún', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'PATATA-GRANDE', grams: 180 }, { ingredientId: 'ATUN-LATA-ACEITE', grams: 90 } ], // Total: 270g
                    tags: [], computed: {}
                },
                'REC-BOWL-SALMON': {
                    id: 'REC-BOWL-SALMON', name: 'Bowl de Salmón y Arroz', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [
                ...
                'REC-MANZANA-POLLO': {
                    id: 'REC-MANZANA-POLLO', name: 'Pollo Asado con Manzana', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [ { ingredientId: 'CHICKEN-BREAST', grams: 140 }, { ingredientId: 'MANZANA-FUJI', grams: 90 } ], // Total: 230g
                    tags: [], computed: {}
                },
                'REC-BOWL-COMPLETO': {
                    id: 'REC-BOWL-COMPLETO', name: 'Bowl Completo Nutritivo', type: 'alimento', itemType: 'receta',
                    raciones: 1,
                    ingredients: [
            ...
            allIds: [
                'calories', 'proteins', 'carbs', 'fats','vitamin_c','vitamin_d','calcium','iron','potassium','sodium',
                'CHICKEN-BREAST', 'MANZANA-FUJI', 'ESPINACAS-FRESCAS', 'SALMON-FILLET', 'PATATA-GRANDE',
                'GALLETAS-CHOCOLATE', 'DORITOS-TEX-MEX', 'ARROZ-BLANCO', 'ATUN-LATA-ACEITE',
                'REC-ENSALADA-ESPINACAS', 'REC-SALMON-PATATAS', 'REC-POLLO-ARROZ', 'REC-ENSALADA-ATUN',
                'REC-SALMON-ESPINACAS', 'REC-POLLO-PATATAS', 'REC-ARROZ-ATUN', 'REC-ENSALADA-POLLO',
                'REC-PATATA-ATUN', 'REC-BOWL-SALMON', 'REC-MANZANA-POLLO', 'REC-BOWL-COMPLETO'
            ]
        },
        profiles: { byId: {
            'DEFAULT-PROFILE': {
    ...
    Object.values(initialData.items.byId).forEach(item => {
        if (item.itemType === 'receta') {
            const { computedTotals, totalGrams } = calculateRecipeTotals(item, initialData.items.byId);
            item.computed = { totals: computedTotals, totalGrams };
        }
    });

    // Handle Migration
    if (persistedState && (persistedState.ingredients || persistedState.recipes || !persistedState.referenceGuides)) {
        ...
        if (migratedState) {
            ...
            Logger.error("Migration failed. Backing up legacy state and starting fresh.");
            localStorage.setItem('atomCanvasState_backup_failed_migration', JSON.stringify(persistedState));
    ...
    if (persistedState) {
        Object.values(finalInitialState.items.byId).forEach(item => {
            if (item.itemType === 'receta') {
                const { computedTotals, totalGrams } = calculateRecipeTotals(item, finalInitialState.items.byId);
                item.computed = { totals: computedTotals, totalGrams };
            }
        });
    }
